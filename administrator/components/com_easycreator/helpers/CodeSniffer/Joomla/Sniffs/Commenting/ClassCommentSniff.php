 ...
 * @version   CVS: $Id: ClassCommentSniff.php 18 2010-11-08 01:10:19Z elkuku $
    ...
    protected function processSince($errorPos)
    {
        ...
        if ($since !== null) {
            ...
            if (empty($content) === true) {
                ...
                && preg_match('/^([0-9]+)\.([0-9]+)/', $content) !== 0) {
                    $error = 'Expected version number to be in the form x.x or x.x.x in @since tag'.$content;
                    $this->currentFile->addError($error, $errorPos);
                }
            }
        }
