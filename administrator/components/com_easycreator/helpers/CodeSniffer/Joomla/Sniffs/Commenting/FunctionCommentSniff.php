 ...
 * @version   CVS: $Id: FunctionCommentSniff.php 18 2010-11-08 01:10:19Z elkuku $
...
//                    $error = 'Expected version number to be in the form x.x.x in @since tag';
                if (preg_match('/^([0-9]+)\.([0-9]+)/', $content) === 0) {
                    $error = 'Expected version number to be in the form x.x in @since tag'.$content;
                    /* CHG NiK END */
                    $this->currentFile->addError($error, $errorPos);
                }
            }
...
/* ADD NiK END */
     ...
     *                          the comment started.
     * @param integer $commentEnd   The position in the stack where
     *                          the comment ended.
     *
     * @return void
     */
...
/* ADD NiK START */
                    ...
                    if ($param->alignsVariableWith($previousParam) === false) {
                        $error = 'The variable names for parameters '.$previousName
                        .' ('.($pos - 1).') and '.$paramName.' ('.$pos.') do not align';
                        $this->currentFile->addError($error, $errorPos);
                    }

                    if ($param->alignsCommentWith($previousParam) === false) {
                        $error = 'The comments for parameters '.$previousName
                        .' ('.($pos - 1).') and '.$paramName.' ('.$pos.') do not align';
                        $this->currentFile->addError($error, $errorPos);
                    }
                */
                    ...
                    if ($typeName !== $suggestedName) {
                        ...
                        if ($suggestedTypeHint !== '' && isset($realParams[($pos - 1)]) === true) {
                            ...
                            if ($typeHint === '') {
                                ...
                                $error = "Expected type hint \"$suggestedTypeHint\"; "
                                ." found \"$typeHint\" for $paramName at position $pos";
