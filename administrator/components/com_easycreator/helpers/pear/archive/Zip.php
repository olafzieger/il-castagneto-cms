 ...
 * @version $Id: Zip.php 18 2010-11-08 01:10:19Z elkuku $
 */
    ...
    &$p_params)
    {
        ...
        for ($j=0; ($j<count($p_list)) && ($v_result==1); $j++)
        {
            ...
            if ((is_file($p_filename)) || ((is_dir($p_filename)) && !$p_remove_all_dir)) {
                // ----- Add the file
                if (($v_result = $this->_addFile($p_filename, $v_header, $p_add_dir, $p_remove_dir
                , $p_remove_all_dir, $p_params)) != 1)
                {
                    // ----- Return status
                    return $v_result;
            ...
            if (is_dir($p_filename))
            {
                ...
                while ($p_hitem = readdir($p_hdir))
                {
                    ...
                    {
                        // ----- Add the file
                        if (($v_result = $this->_addFile($v_path.$p_hitem, $v_header, $p_add_dir
                        , $p_remove_dir, $p_remove_all_dir, $p_params)) != 1)
                        {
                            // ----- Return status
                            return $v_result;
                        }
                    ...
                    else
                    {
                        ...
                        $v_result = $this->_addFileList($p_temp_list, $p_result_list, $p_add_dir
    ...
    */
        ...
        for ($i=0, $v_nb_extracted=0; $i<$v_central_dir['entries']; $i++) {
            ...
            && ($p_params[ARCHIVE_ZIP_PARAM_BY_NAME] != 0)) {
                ...
                $j++) {
                    ...
                    if (substr($p_params[ARCHIVE_ZIP_PARAM_BY_NAME][$j], -1) == "/") {
                        ...
                        , strlen($p_params[ARCHIVE_ZIP_PARAM_BY_NAME][$j])) == $p_params[ARCHIVE_ZIP_PARAM_BY_NAME][$j]))
                        {
                            $v_extract = true;
                        }
                    }
            ...
            if ($v_extract)
            {
                ...
                if ($p_params[ARCHIVE_ZIP_PARAM_EXTRACT_AS_STRING]) {
                    ...
                    // ----- Extracting the file
                    if (($v_result = $this->_extractFile($v_header, $p_path, $p_remove_path
                    , $p_remove_all_path, $p_params)) != 1)
                    {
                        // ----- Close the zip file
                        $this->_closeFd();
    ...
    function _extractFile(&$p_entry, $p_path, $p_remove_path, $p_remove_all_path, &$p_params)
    {
        ...
        && ($p_params[ARCHIVE_ZIP_PARAM_PRE_EXTRACT] != '')) {
            ...
            // Here I do not use call_user_func() because I need to send a reference to the
            // header.
            eval('$v_result = '.$p_params[ARCHIVE_ZIP_PARAM_PRE_EXTRACT]
        ...
        && ($p_params[ARCHIVE_ZIP_PARAM_POST_EXTRACT] != '')) {
            ...
            // Here I do not use call_user_func() because I need to send a reference to the
            // header.
            eval('$v_result = '.$p_params[ARCHIVE_ZIP_PARAM_POST_EXTRACT]
    ...
    function _readFileHeader(&$p_header)
    {
        ...
        function _readCentralFileHeader(&$p_header)
        {
            ...
            // ----- Extract the values
            $p_header = unpack('vversion/vversion_extracted/vflag/vcompression/vmtime/vmdate/'
                    ...
                    $j++) {
                        ...
                        if (substr($p_params[ARCHIVE_ZIP_PARAM_BY_NAME][$j], -1) == "/") {
                            ...
                            if((strlen($v_header_list[$v_nb_extracted]['stored_filename'])
                             > strlen($p_params[ARCHIVE_ZIP_PARAM_BY_NAME][$j]))
